...
st.markdown(
    """
    <style>
    /* Força a remoção de qualquer margem e padding superior no app do Streamlit */
    .stMainBlockContainer, .block-container {
        padding-top: 0rem !important;
        margin-top: -30px !important;
    }
    
    /* Remove o espaçamento do topo do container principal de navegação */
...
if os.path.exists("logo.png"):
    ...
    st.warning(
        "⚠️ Arquivo 'logo.png' não encontrado no diretório do projeto. Verifique se ele foi enviado para o repositório."
    )
...
if pagina_selecionada == "📊 Dados por Região (Detalhado)":
    try:
        ...
        if aba_selecionada == "Escolha a Região...":
            ...
            if not nomes_titulos:
                st.warning(
                    "⚠️ Nenhum padrão 'FATO COMUNICADO' localizado nesta guia."
                )
            ...
            if titulo_selecionado == "Escolha a Ocorrência...":
                ...
                for i in range(df_bloco.shape[1]):
                    celula_cabecalho = str(
                        df_bruto.iloc[linha_cabecalho_colunas, i]
                    ).strip()
                    nomes_colunas.append(
                        celula_cabecalho
                        if celula_cabecalho.lower() != "nan" and celula_cabecalho != ""
                        else f"Unnamed_{i}"
                    )
                ...
                for col in df_bloco.columns:
                    ...
                    if "ano" in nome_coluna:
                        coluna_ano_real = col
                    if "caso" in nome_coluna or "real" in nome_coluna:
                ...
                if coluna_casos_reais and not df_exibicao.empty:
                    vetor_casos = (
                        df_exibicao[coluna_casos_reais]
                        .astype(str)
                        .str.replace(".", "", regex=False)
                        .str.replace(",", ".", regex=False)
                    )
                    numeros_finais = pd.to_numeric(
                        vetor_casos, errors="coerce"
                    ).to_numpy()
                    ...
                    for idx in range(1, len(numeros_finais)):
                        ...
                        if pd.isna(anterior) or pd.isna(atual) or anterior == 0:
                            ...
                            if calc_arredondado == 0.0:
                                ...
                                strings_var_final.append(
                                    f"{'+' if calc_arredondado > 0 else ''}{calc_arredondado:.1f}%"
                                )
                    ...
                    if "VARIAÇÃO %" in todas_cols:
                        todas_cols.remove("VARIAÇÃO %")
                    if "Taxa por 1.000 Hab." in todas_cols:
                        todas_cols.remove("Taxa por 1.000 Hab.")
                ...
                for coluna in df_exibicao.columns:
                    df_exibicao[coluna] = (
                        ...
                        .astype(str)
                        .str.strip()
                        .replace(["", "None", "none", "NaN", "nan", None], "-----")
                    )
                ...
                if not df_exibicao.empty:
                    ...
                    if coluna_casos_reais:
                        ...
                        tipo_grafico = st.radio(
                            ...
                            options=["Linha", "Barra"],
                            horizontal=True,
                            key=f"radio_{aba_selecionada}_{idx_selecionado}",
                        )
                        ...
                        if mapeamento_checkboxes:
                            ...
                            for i, label in enumerate(opcoes_labels):
                                with cols_checkboxes[i]:
                                    if st.checkbox(
                                        label,
                                        key=f"c_{label}_{aba_selecionada}_{idx_selecionado}",
                                    ):
                                        colunas_selecionadas_reais.append(
                                            mapeamento_checkboxes[label]
                                        )

                        st.markdown(
                            '<div class="legenda-container"><div class="legenda-item"><div'
                            ' class="legenda-cor" style="background-color:'
                            ' #007bff;"></div><span>Casos Reais</span></div><div'
                            ' class="legenda-item"><div class="legenda-cor"'
                            ' style="background-color:'
                            ' #28a745;"></div><span>Projeções PA</span></div><div'
                            ' class="legenda-item"><div class="legenda-cor"'
                            ' style="background-color:'
                            ' #dc3545;"></div><span>Projeções PG</span></div></div>',
                            unsafe_allow_html=True,
                        )

                        df_grafico = df_exibicao.copy()
                        colunas_para_plotar = [coluna_casos_reais] + (
                            colunas_selecionadas_reais
                        )
                        ...
                        df_grafico[coluna_ano_real] = pd.to_numeric(
                            df_grafico[coluna_ano_real], errors="coerce"
                        )
                        df_grafico = df_grafico.dropna(subset=[coluna_ano_real]).sort_values(
                            by=coluna_ano_real
                        )
                        ...
                        for col in colunas_para_plotar:
                            ...
                            if col == coluna_casos_reais:
                                mapeamento_cores[col] = "#007bff"
                            elif "pa" in c_low:
                                mapeamento_cores[col] = "#28a745"
                            elif "pg" in c_low:
                                mapeamento_cores[col] = "#dc3545"
                            else:
                                mapeamento_cores[col] = "#6c757d"

                        df_longo = df_grafico.melt(
                            id_vars=[coluna_ano_real],
                            value_vars=colunas_para_plotar,
                            var_name="Métrica",
                            value_name="Valores",
                        )
                        ...
                        if not df_longo.dropna(subset=["Valores"]).empty:
                            base = alt.Chart(df_longo).encode(
                                ...
                                color=alt.Color(
                                    "Métrica:N",
                                    scale=alt.Scale(
                                        domain=list(mapeamento_cores.keys()),
                                        range=list(mapeamento_cores.values()),
                                    ),
                                    legend=None,
                                ),
                            )
                            chart = (
                                ...
                                else base.mark_bar().encode(xOffset="Métrica:N")
                            )
                            st.markdown(
                                '<div class="grafico-container">', unsafe_allow_html=True
                            )
                            st.altair_chart(
                                chart.properties(width=900, height=450).interactive(),
    ...
    # --- PÁGINA DE COMPARATIVO GERAL (MÉDIA GERAL DOS ANOS POR OCORRÊNCIA) ---
    st.subheader("🗺️ Média Geral Histórica por Tipo de Ocorrência e Região")
    st.write(
        "Selecione o tipo de ocorrência abaixo para visualizar a tabela com a"
        " **média geral proporcional** (taxa média por 1.000 habitantes femininas"
        " considerando todos os anos disponíveis) de cada região."
    )

    try:
        primeira_regiao_gid = list(MAPA_REGIOES.values())[0]
        df_modelo = carregar_dados_brutos(CHAVE_PLANILHA, primeira_regiao_gid)

        nomes_titulos_geral = []
        for idx, row in df_modelo.iterrows():
            celula_texto = str(row.iloc[0]).upper()
            if "FATO COMUNICADO" in celula_texto or "OCORRÊNCIA" in celula_texto:
                nomes_titulos_geral.append(str(row.iloc[0]).strip())

        if not nomes_titulos_geral:
            st.warning("⚠️ Não foi possível carregar as ocorrências padrão.")
        else:
            ocorrencia_escolhida = st.selectbox(
                "Selecione a Ocorrência para Média Geral:",
                options=["Escolha a Ocorrência..."] + nomes_titulos_geral,
            )

            if ocorrencia_escolhida != "Escolha a Ocorrência...":
                st.write("---")

                with st.spinner(
                    "Calculando as médias gerais proporcionais de todas as"
                    " regiões..."
                ):
                    dados_comparativos = []
                    ...
                    for nome_reg, gid_reg in MAPA_REGIOES.items():
                        try:
                            df_b = carregar_dados_brutos(CHAVE_PLANILHA, gid_reg)

                            # Extrai população (Célula E3)
                            pop = 1000000
                            if df_b.shape[0] > 2 and df_b.shape[1] > 4:
                                num_str = re.sub(r"\D", "", str(df_b.iloc[2, 4]))
                                if num_str:
                                    pop = float(num_str)

                            # Localiza a linha correspondente à ocorrência escolhida
                            idx_titulo = -1
                            for idx, row in df_b.iterrows():
                                if str(row.iloc[0]).strip() == ocorrencia_escolhida:
                                    idx_titulo = idx
                                    break

                            if idx_titulo != -1 and idx_titulo + 2 < len(df_b):
                                prox_titulo = len(df_b)
                                for other_idx in range(idx_titulo + 1, len(df_b)):
                                    txt_out = str(df_b.iloc[other_idx, 0]).upper()
                                    if "FATO COMUNICADO" in txt_out or "OCORRÊNCIA" in txt_out:
                                        prox_titulo = other_idx
                                        break

                                df_sub = df_b.iloc[
                                    idx_titulo + 2 : prox_titulo
                                ].dropna(how="all")

                                col_caso = None
                                for c_idx in range(df_b.shape[1]):
                                    c_name = str(
                                        df_b.iloc[idx_titulo + 1, c_idx]
                                    ).lower()
                                    if "caso" in c_name or "real" in c_name:
                                        col_caso = c_idx
                                        break

                                if col_caso is not None and not df_sub.empty:
                                    taxas_anuais = []
                                    casos_totais = []
                                    for row_i, r_val in df_sub.iterrows():
                                        ano_val = str(r_val.iloc[0]).strip()
                                        caso_val_raw = str(r_val.iloc[col_caso]).strip()
                                        if ano_val.isdigit() and caso_val_raw not in [
                                            "",
                                            "nan",
                                            "None",
                                            "-----",
                                            ...
                                            num_casos = pd.to_numeric(
                                                caso_val_raw.replace(".", "").replace(
                                                    ",", "."
                                                ),
                                                errors="coerce",
                                            )
                                            if not pd.isna(num_casos):
                                                taxa_anual = (num_casos / pop) * 1000
                                                taxas_anuais.append(taxa_anual)
                                                casos_totais.append(num_casos)

                                    if taxas_anuais:
                                        media_taxa = sum(taxas_anuais) / len(taxas_anuais)
                                        media_casos = sum(casos_totais) / len(casos_totais)
                                        dados_comparativos.append({
                                            "Região": nome_reg,
                                            "População Feminina": int(pop),
                                            "Média de Casos Reais": round(media_casos, 1),
                    ...
                    if dados_comparativos:
                        ...
                        # Ordena da maior média da taxa para a menor
                        df_comp = df_comp.sort_values(
                            by="Média da Taxa por 1.000 Hab.", ascending=False
                        ).reset_index(drop=True)

                        st.subheader(
                            f"📋 Tabela de Média Geral Histórica — {ocorrencia_escolhida}"
                        )
                        st.dataframe(df_comp, use_container_width=True, hide_index=True)
                    else:
                        st.warning(
                            "⚠️ Não foram encontrados dados suficientes para esta"
                            " ocorrência em todas as regiões."
                        )
            else:
                st.info(
                    "ℹ️ Selecione uma ocorrência acima para visualizar a média geral"
                    " por região."
                )
    except Exception as e:
        st.error(f"Erro ao carregar o comparativo: {e}")
