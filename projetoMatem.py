...
pagina_selecionada = st.radio(
    "Modo de Navegação:",
    ["📊 Dados por Região (Detalhado)", "🗺️ Comparativo Geral entre Regiões"],
    horizontal=True
)
...
if pagina_selecionada == "📊 Dados por Região (Detalhado)":
    ...
    st.write("Selecione o tipo de ocorrência abaixo para visualizar a tabela com a **média geral proporcional** (taxa média por 1.000 habitantes femininas considerando todos os anos disponíveis) de cada região.")
    ...
    try:
        ...
        if not nomes_titulos_geral:
            ...
            ocorrencia_escolhida = st.selectbox("Selecione a Ocorrência para Média Geral:", options=["Escolha a Ocorrência..."] + nomes_titulos_geral)
            
            if ocorrencia_escolhida != "Escolha a Ocorrência...":
                st.write("---")
                
                with st.spinner("Calculando as médias gerais proporcionais de todas as regiões..."):
                    dados_comparativos = []
                    ...
                    for nome_reg, gid_reg in MAPA_REGIOES.items():
                        try:
                            ...
                            if idx_titulo != -1 and idx_titulo + 2 < len(df_b):
                                ...
                                df_sub = df_b.iloc[idx_titulo + 2:prox_titulo].dropna(how="all")
                                ...
                                if col_caso is not None and not df_sub.empty:
                                    ...
                                    for row_i, r_val in df_sub.iterrows():
                                        ...
                                        if ano_val.isdigit() and caso_val_raw not in ["", "nan", "None", "-----"]:
                                            num_casos = pd.to_numeric(caso_val_raw.replace(".", "").replace(",", "."), errors="coerce")
                    ...
                    if dados_comparativos:
                        ...
                        df_comp = df_comp.sort_values(by="Média da Taxa por 1.000 Hab.", ascending=False).reset_index(drop=True)
                        
                        st.subheader(f"📋 Tabela de Média Geral Histórica — {ocorrencia_escolhida}")
                        st.dataframe(df_comp, use_container_width=True, hide_index=True)
                    else:
                        st.warning("⚠️ Não foram encontrados dados suficientes para esta ocorrência em todas as regiões.")
            else:
                st.info("ℹ️ Selecione uma ocorrência acima para visualizar a média geral por região.")
    except Exception as e:
